...
export default function Pricing() {
  ...
  useEffect(() => {
    ...
    if (selectedPlan) document.addEventListener("mousedown", handleClickOutside);
    return () => document.removeEventListener("mousedown", handleClickOutside);
  }, [selectedPlan]);

  // ✅ close on Escape key
  useEffect(() => {
    const handleEsc = (e: KeyboardEvent) => e.key === "Escape" && setSelectedPlan(null);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-[-100px] right-[-120px] w-[280px] h-[280px] bg-[#F8BBD0]/40 blur-3xl rounded-full" />
      <div className="absolute bottom-[-100px] left-[-120px] w-[300px] h-[300px] bg-[#E1BEE7]/40 blur-3xl rounded-full" />
      ...
      <div className="relative container mx-auto px-6">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16">
          ...
          <p className="text-[#64748B] mt-4 text-lg leading-relaxed">
            We’re currently in Early Access. Explore GarbhSakhi’s premium
            features free of charge — professional plans will go live after our
            public launch.
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {plans.map((p, i) => (
            ...
            >
              ...
              <h3 className="font-extrabold text-xl text-[#1E293B] mt-4">
                {p.name}
              </h3>
              <div className="text-4xl font-extrabold my-3 text-[#E91E63]">
                {p.price}
                <span className="text-base text-[#64748B] font-medium">
                  {p.price !== "Custom" && "/month"}
                </span>
              </div>
              {p.note && (
                <p className="text-sm text-[#9CA3AF] italic mb-4">{p.note}</p>
              )}
        ...
        <p className="text-center mt-10 text-[#94A3B8] text-sm">
          ...
          Clinics in our early-access program will receive exclusive lifetime
          discounts.
        </p>
      </div>
